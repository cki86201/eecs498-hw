...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    xp = x_train.view(num_train, -1)
    yp = x_test.view(num_test, -1)
    for i in range(num_train):
      dists[i] += ((yp - xp[i]) ** 2).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xp = x_train.view(num_train, -1)
    yp = x_test.view(num_test, -1)
    dists += (xp ** 2).sum(dim=1).view(-1, 1)
    dists += (yp ** 2).sum(dim=1)
    dists -= 2 * torch.mm(xp, yp.transpose(0, 1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y_pred += torch.mode(y_train[torch.topk(dists, k, dim=0, largest=False)[1]], dim=0)[0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        xp_test = x_train_folds[i]
        xp_train = torch.cat([x_train_folds[j] for j in range(num_folds) if i != j])
        yp_test = y_train_folds[i]
        yp_train = torch.cat([y_train_folds[j] for j in range(num_folds) if i != j])
        classifier = KnnClassifier(xp_train, yp_train)
        k_to_accuracies[k].append(classifier.check_accuracy(xp_test, yp_test, k=k, quiet=True))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
