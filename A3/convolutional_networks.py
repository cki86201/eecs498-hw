...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        px = torch.nn.functional.pad(x, (pad, pad, pad, pad))
        N, C, H, W = x.shape
        F, _, HH, WW = w.shape
        Hp = 1 + (H + 2 * pad - HH) // stride
        Wp = 1 + (W + 2 * pad - WW) // stride
        out = torch.zeros((N, F, Hp, Wp), dtype=px.dtype, device=px.device)
        out += b.view(1, -1, 1, 1)
        for j in range(Hp):
          for k in range(Wp):
            for jx in range(HH):
              for kx in range(WW):
                out[:,:,j,k] += torch.mm(px[:,:,j * stride + jx,k * stride + kx],
                  w[:,:,jx,kx].t())
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
          ...
          for k in range(Wp):
            for jx in range(HH):
              for kx in range(WW):
                jpx = j * stride + jx
                kpx = k * stride + kx
                dw[:,:,jx,kx] += torch.mm(dout[:,:,j,k].t(), px[:,:,jpx,kpx])
                if 0 <= jpx-pad and jpx-pad < H and 0 <= kpx-pad and kpx-pad < W:
                  dx[:,:,jpx-pad,kpx-pad] += torch.mm(dout[:,:,j,k], w[:,:,jx,kx])
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for h in range(Hp):
          for w in range(Wp):
            out[:, :, h, w] = x[:, :, \
            h*stride:h*stride+pool_height, w*stride:w*stride+pool_width].amax(dim=(2,3))
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for h in range(Hp):
          for w in range(Wp):
            px = x[:, :, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width]
            max_t = px.flatten(-2).argmax(dim=2)
            for i in range(N):
              for j in range(C):
                hp = h*stride + max_t[i][j].item() // pool_height
                wp = w*stride + max_t[i][j].item() % pool_height
                dx[i][j][hp][wp] = dout[i][j][h][w]
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh2, dw3, db3 = Linear.backward(dscores, cache3)
        grads['b3'] = db3
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers):
          if i + 1 < self.num_layers:
            # weights of conv
            cin = num_filters[i - 1] if i > 0 else C
            if weight_scale == 'kaiming':
              self.params[f'W{i+1}'] = kaiming_initializer(cin, num_filters[i], 3, dtype=dtype, device=device)
            else:
              self.params[f'W{i+1}'] = torch.randn(num_filters[i], cin, 3, 3, dtype=dtype, device=device) * weight_scale
            ...
            # weight of fc
            rate = 2 ** len(max_pools)
            M = num_filters[i - 1] * (H // rate) * (W // rate)
            if weight_scale == 'kaiming':
              self.params[f'W{i+1}'] = kaiming_initializer(M, num_classes, dtype=dtype, device=device)
            else:
              self.params[f'W{i+1}'] = torch.randn(M, num_classes, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        xp = X
        for i in range(self.num_layers):
          Wi, bi = self.params[f'W{i+1}'], self.params[f'b{i+1}']
          if i + 1 < self.num_layers:
            # conv - (BN?) - ReLU - (Pool?)
            if self.batchnorm:
              gamma, beta = self.params[f'gamma{i+1}'], self.params[f'beta{i+1}']
              if i in self.max_pools:
                xp, cache = Conv_BatchNorm_ReLU_Pool.forward(xp, Wi, bi, gamma, beta, conv_param, self.bn_params[i], pool_param)
              else:
                xp, cache = Conv_BatchNorm_ReLU.forward(xp, Wi, bi, gamma, beta, conv_param, self.bn_params[i])
            else:
              if i in self.max_pools:
                xp, cache = Conv_ReLU_Pool.forward(xp, Wi, bi, conv_param, pool_param)
              else:
                xp, cache = Conv_ReLU.forward(xp, Wi, bi, conv_param)
          else:
            # fc
            xp, cache = Linear.forward(xp, Wi, bi)
        ...
        for i in range(self.num_layers-1, -1, -1):
          ...
          if i + 1 < self.num_layers:
            ...
            if self.batchnorm:
              if i in self.max_pools:
                dxp, dWi, dbi, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dxp, cache)
              else:
                dxp, dWi, dbi, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dxp, cache)
              grads[f'gamma{i+1}'] = dgamma
              grads[f'beta{i+1}'] = dbeta
            else:
              if i in self.max_pools:
                dxp, dWi, dbi = Conv_ReLU_Pool.backward(dxp, cache)
              else:
                dxp, dWi, dbi = Conv_ReLU.backward(dxp, cache)
          else:
            # fc
            dxp, dWi, dbi = Linear.backward(dxp, cache)
          grads[f'W{i+1}'] = dWi + 2 * self.reg * Wi
          grads[f'b{i+1}'] = dbi
          loss += self.reg * torch.sum(Wi * Wi)
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict,
                  ...
                  print_every=80, device=device)
    '''
    model = DeepConvNet(input_dims=input_dims, num_classes=10,
                      num_filters=[8, 8, 32, 32, 70, 70],
                      max_pools=[0, 2, 4],
                      weight_scale='kaiming',
                      batchnorm=True,
                      reg=2e-3, 
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            xmu = x - mu
            var = torch.sum(xmu ** 2, dim=0) / N
            xhat = xmu / torch.sqrt(var + eps)
            out = xhat * gamma + beta
            running_mean = momentum * running_mean + (1 - momentum) * mu
            running_var = momentum * running_var + (1 - momentum) * var
            cache = (x, mu, xmu, var, eps, xhat, gamma)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        (x, mu, xmu, var, eps, xhat, gamma) = cache
        N, D = x.shape
        dbeta = dout.sum(dim=0)
        dgamma = (dout * xhat).sum(dim=0)
        dxhat = dout * gamma
        dvar = (dxhat * xmu).sum(dim=0) * -0.5 * (var + eps) ** -1.5
        dxmu = dxhat * (var + eps) ** -0.5 + dvar / N * 2 * xmu
        dx = dxmu - dxmu.mean(dim=0)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        dbeta = dout.sum(dim=0)
        dgamma = (dout * xhat).sum(dim=0)
        dx = (var + eps) ** -0.5 * gamma * (dout - dout.mean(dim=0) \
        - (xmu - xmu.mean(dim=0)) * (dout * xmu).mean(dim=0) / (var + eps))
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        xp = x.permute(0, 2, 3, 1).reshape(-1, C)
        out, cache = BatchNorm.forward(xp, gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        doutp = dout.permute(0, 2, 3, 1).reshape(-1, C)
        dx, dgamma, dbeta = BatchNorm.backward(doutp, cache)
