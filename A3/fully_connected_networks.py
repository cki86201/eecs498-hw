...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        h1, cache_1 = Linear_ReLU.forward(X, W1, b1)
        scores, cache_2 = Linear.forward(h1, W2, b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dh, dw2, db2 = Linear.backward(dscore, cache_2)
        grads['b2'] = db2
        grads['W2'] = dw2 + self.reg * 2 * W2
        dx, dw1, db1 = Linear_ReLU.backward(dh, cache_1)
        grads['b1'] = db1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          wi = 'W' + str(i+1)
          bi = 'b' + str(i+1)
          nsize = input_dim if i == 0 else hidden_dims[i - 1]
          msize = num_classes if i == self.num_layers - 1 else hidden_dims[i]
          self.params[wi] = torch.normal(0, weight_scale, size=(nsize, msize), dtype=dtype, device=device)
          self.params[bi] = torch.zeros(msize, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          ...
          Wx, bx = self.params[wi], self.params[bi]
          if i + 1 < self.num_layers:
            hx, cache_x = Linear_ReLU.forward(Xv, Wx, bx)
            if self.use_dropout:
              hx, cache_u = Dropout.forward(hx, self.dropout_param)
              cache2.append(cache_u)
          else:
            hx, cache_x = Linear.forward(Xv, Wx, bx)
          Xv = hx
        ...
        #####################################################################
        # Replace "pass" statement with your code
        loss, dscore = softmax_loss(Xv, y)
        dout = dscore
        for i in range(self.num_layers-1, -1, -1):
          wi, bi = 'W' + str(i + 1), 'b' + str(i + 1)
          Wx, bx = self.params[wi], self.params[bi]
          loss += self.reg * torch.sum(Wx * Wx)
          if i + 1 < self.num_layers:
            if self.use_dropout:
              dout = Dropout.backward(dout, cache2[i])
            dx, dw, db = Linear_ReLU.backward(dout, cache[i])
          else:
            dx, dw, db = Linear.backward(dout, cache[i])
          grads[wi] = dw + 2 * self.reg * Wx
          grads[bi] = db
          dout = dx
        ###########################################################
        #                   END OF YOUR CODE                      #
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=200, reg=0.002, dtype=dtype, device=device)
    #############################################################
