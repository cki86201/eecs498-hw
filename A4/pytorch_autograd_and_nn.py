...
def three_layer_convnet(x, params):
  ...
  x = F.relu(F.conv2d(x, conv_w1, bias=conv_b1, stride=1, padding=2))
  x = F.relu(F.conv2d(x, conv_w2, bias=conv_b2, stride=1, padding=1))
  x = flatten(x, 1)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  H1 = 1 + (H + 2*2 - kernel_size_1) // 1
  W1 = 1 + (W + 2*2 - kernel_size_1) // 1
  ...
  H2 = 1 + (H1 + 2*1 - kernel_size_2) // 1
  W2 = 1 + (W1 + 2*1 - kernel_size_2) // 1
  L = H2 * W2 * channel_2
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, L, dtype=dtype, device=device))
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    for args in stage_args:
      blocks.append(ResNetStage(*args, block=block))
