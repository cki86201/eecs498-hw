...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    b1 = torch.mm(x, Wx)
    b2 = torch.mm(prev_h, Wh)
    next_h = torch.tanh(b1 + b2 + b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    ds = (torch.ones_like(next_h) - next_h ** 2) * dnext_h
    db = ds.sum(dim=0)
    dx = torch.mm(ds, Wx.t())
    dprev_h = torch.mm(ds, Wh.t())
    dWx = torch.mm(x.t(), ds)
    dWh = torch.mm(prev_h.t(), ds)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        if self.cell_type == 'attention':
          ...
          self.Wih = torch.nn.Conv2d(1280, hidden_dim, 1, device=device, dtype=dtype)
        else:
          self.Wih = nn.Linear(1280, hidden_dim, device=device, dtype=dtype)
        ...
        self.Why = nn.Linear(hidden_dim, vocab_size, dtype=dtype, device=device)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def forward(self, images, captions):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        h0 = self.Wih(self.feature_extractor.extract_mobilenet_feature(images))
        captions_in = self.word_embed(captions_in)
        hs = self.RNN(captions_in, h0)
        out = self.Why(hs)
    ...
    def sample(self, images, max_length=15):
        ...
        # Replace "pass" statement with your code
        if self.cell_type == 'attention':
          A = self.Wih(self.feature_extractor.extract_mobilenet_feature(images))
          prev_h = A.mean(dim=(2, 3))
          prev_c = prev_h.clone().detach()
        else:
          prev_h = self.Wih(self.feature_extractor.extract_mobilenet_feature(images))
        
        if self.cell_type == 'lstm':
          prev_c = torch.zeros_like(prev_h)
        x = self.word_embed([self._start for _ in range(N)])
        for i in range(max_length):
          if self.cell_type == 'rnn':
            ...
            attn_weights_all[:, i] = attn_w
          yi = self.Why(prev_h)
          idxs = yi.argmax(dim = 1)
          captions[:, i] = idxs
          x = self.word_embed(idxs)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        if self.cell_type == 'attention':
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    H = prev_h.shape[-1]
    A[:, :3*H] = torch.sigmoid(A[:, :3*H])
    A[:, 3*H:] = torch.tanh(A[:, 3*H:])
    next_c = prev_c * A[:, H:2*H] + A[:, :H] * A[:, 3*H:]
    next_h = A[:, 2*H:3*H] * torch.tanh(next_c)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_h, prev_c = h0, c0
    for i in range(T):
      prev_h, prev_c = lstm_step_forward(x[:, i], prev_h, prev_c, Wx, Wh, b)
      h[:, i] = prev_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def dot_product_attention(prev_h, A):
    ...
    # Replace "pass" statement with your code
    #import math
    attn_weights = torch.bmm(prev_h.reshape(N, 1, H), A.reshape(N, H, -1)).reshape(N, -1)
    attn_weights /= math.sqrt(H)
    attn_weights = F.softmax(attn_weights, dim=1)
    attn = torch.bmm(A.reshape(N, H, -1), attn_weights.reshape(N, -1, 1)).reshape(N, -1)
