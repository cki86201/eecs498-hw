...
def compute_saliency_maps(X, y, model):
  ...
  out = model(X)
  scores = out[range(0, y.shape[0]), y].sum()
  scores.backward()
  saliency = X.grad.data.sum(dim=1)
  ##############################################################################
  #               END OF YOUR CODE                                             #
  ##############################################################################
  return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
    res = model(X_adv)
    if verbose:
      print('Iteration %d: target score %.3f, max score %.3f' % (i, res[0][target_y].item(), res.max().item()))
    if res[0].argmax() == target_y:
      break
    res[:, target_y].backward()
    with torch.no_grad():
      X_adv += learning_rate * X_adv.grad / (X_adv.grad * X_adv.grad).sum()
      X_adv.grad.zero_()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
