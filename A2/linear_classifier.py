...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.3e-2, 0.4e-2, 0.5e-2, 0.6e-2, 0.7e-2]
    regularization_strengths = [0.9e-3, 0.8e-3, 0.7e-3, 1e-3, 1.1e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
      scores = W.t().mv(X[i])
      scores = torch.exp(scores - max(scores))
      loss += -math.log(scores[y[i]] / scores.sum())
      tv = scores / scores.sum()
      tv[y[i]] -= 1
      dW += torch.mm(X[i].view(-1, 1), tv.view(1, -1))
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    R = torch.mm(X, W)
    R -= torch.max(R, dim=1, keepdim=True)[0]
    R = torch.exp(R)
    R /= R.sum(dim=1, keepdim=True)
    loss += -(torch.log(R[range(0, num_train), y])).sum()
    R[range(0, num_train), y] -= 1
    dW += torch.mm(X.t(), R)
    loss /= num_train
    dW /= num_train
    loss += reg * torch.sum(W * W)
