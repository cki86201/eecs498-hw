...
def nn_forward_backward(
    ...
    scores_vec = scores[range(0, N), y]
    scores_vec = -torch.log(scores_vec)
    loss = scores_vec.mean().double()
    ...
    # Replace "pass" statement with your code
    s_grad = scores
    s_grad[range(0, N), y] -= 1
    s_grad /= N
    grads['b2'] = s_grad.sum(dim=0)
    grads['W2'] = torch.mm(h1.t(), s_grad) + 2 * reg * W2
    h_grad = torch.mm(s_grad, W2.t())
    h_grad[h1 <= 0] = 0
    grads['b1'] = h_grad.sum(dim=0)
    grads['W1'] = torch.mm(X.t(), h_grad) + 2 * reg * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    y_pred = nn_forward_backward(params, X).argmax(dim=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1.2, 1.0, 0.8]
    learning_rate_decays = [0.97]
    hidden_sizes = [128]
    regularization_strengths = [1.2e-3, 1.1e-3, 1e-3, 0.9e-3, 0.8e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
      ...
      for lr in learning_rates:
        for lrd in learning_rate_decays:
          for reg in regularization_strengths:
            print('hd =', hd, ', lr =', lr, ',lrd =', lrd, ',reg = ', reg)
            net = TwoLayerNet(3 * 32 * 32, hd, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
            ...
            val_acc = (net.predict(data_dict['X_val']) == data_dict['y_val']).double().mean().item()
            train_acc = (net.predict(data_dict['X_train']) == data_dict['y_train']).double().mean().item()
            results[(hd, lr, lrd, reg)] = train_acc, val_acc
    ...
    y_scatter = [math.log10(x[3]) for x in results]
    marker_size = 100
